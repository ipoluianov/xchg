...
func (c *HttpServer) processX(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	...
	if !limiterOK {
		...
		destAddress := r.FormValue("dest")
		data := r.FormValue("d")
		message, err = c.core.Execute(r.Context(), address, destAddress, []byte(data), time.Duration(c.config.Http.LongPollingTimeoutMs)*time.Millisecond)
	}
